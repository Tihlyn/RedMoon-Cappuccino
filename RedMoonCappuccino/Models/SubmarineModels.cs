...
///
/// That file is generated by <c>subs/build_dataset.mjs</c> from the community
/// planner page in <c>subs/</c> plus the breakpoint spreadsheet in
/// <c>resources/subs_data/</c>; run the script after either source changes
/// rather than editing the JSON. Field names are the sources' own short keys so
...
public sealed class SubmarineSector
{
    ...
    /// <summary>
    /// Submarine rank needed to plot this sector, or 0 when the dataset does not
    /// say. Only the rank 130+ sectors carry it; for the rest the game's own
    /// sheet is the source.
    /// </summary>
    [JsonPropertyName("rk")] public int RankRequirement { get; set; }
...
public sealed class SubmarineDrop
{
    ...
    /// <summary>
    /// True when no voyages have been recorded for this drop yet and the
    /// numbers above are the dataset's placeholder — one unit at a 50% chance.
    /// The UI must not present these as measured.
    /// </summary>
    [JsonPropertyName("est")] public bool Estimated { get; set; }
}
